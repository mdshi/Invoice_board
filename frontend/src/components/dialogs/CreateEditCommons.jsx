...
const InvoiceFieldSelectStatus = ({
  field, // { name, value, onChange, onBlur }
  form: { touched, errors }, // also values, setXXXX, handleXXXX, dirty, isValid, status, etc.
  ...props
}) => {
  ...
  return (
    <FormControl className={classes.formControl} variant="outlined" fullWidth>
      <TextField
        ...
        {...props}
      >
        <option key={0} value={INVOICE_STATUSES.NOT_PAID}>
          Not Done
        </option>
        <option key={1} value={INVOICE_STATUSES.PAID}>
          Done
        </option>
      </TextField>
    </FormControl>
  ...
  return ({
    ...
    <Form>
      <StyledDialogContent>
        ...
        <Field type="text" name="name" title="Name" component={InvoiceFieldInput} />
        <ErrorMessage name="name" render={errorText} />
        <Field type="text" name="status" title="Status" component={InvoiceFieldSelectStatus} />
        <ErrorMessage name="status" render={errorText} />
        <Field multiline type="text" name="description" title="Description" component={InvoiceFieldInput} />
